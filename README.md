...
[![Transformers](https://img.shields.io/badge/🤗%20Transformers-4.20+-green.svg)](https://huggingface.co/transformers/)
...
- **损失形式** (`loss_type="kto"`)：本质上是对 DPO 损失的加权 Sigmoid 交叉熵形式，可写为
  ...
  $$
  L_{KTO} = - \big[ w^+ \cdot \log\sigma(\beta (r_\theta(x,y_w) - r_\theta(x,y_l))) + w^- \cdot \log(1-\sigma(\beta (r_\theta(x,y_w) - r_\theta(x,y_l))) ) \big]
  $$
  其中 $w^+=\text{desirable\_weight}$，$w^-=\text{undesirable\_weight}$。
